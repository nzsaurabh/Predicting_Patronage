...
save(tsdata, file = paste0(datadir,"tsdata.rda"))



# create function for time series analysis

tsmodel.fun <- function(x){
...
# predictions
tspred <- as.vector(exp(ts.output$tsforecast$mean))

low95 <- as.vector(exp(ts.output$tsforecast$lower))

up95 <- as.vector(exp(ts.output$tsforecast$upper))
...
# apply ts function
tsanalysis <- lapply(tsdata, FUN = tsmodel.fun)

# save the analysis
save(tsanalysis, file = "tsanalysis.rda")
